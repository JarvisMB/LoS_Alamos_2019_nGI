...
def normalization(stack_im,stack_ob,xROI=xROI,yROI=yROI,widthROI=widthROI,heightROI=heightROI,show=True):
    ...
    stack_im_ar = []
    stack_im = stack_im.astype(float)
    for l in stack_im:
        stack_im_ar.append(l/(l[yROI:yROI+heightROI+1,xROI:xROI+widthROI+1].sum()/area))
        
    #stack_ob_ar = [l/(l[yROI:yROI+heightROI+1,xROI:xROI+widthROI+1].sum()/area) for l in stack_ob] 
    stack_ob_ar = []
    
    stack_ob = stack_ob.astype(float)
    for l in stack_ob:
        stack_ob_ar.append(l/(l[yROI:yROI+heightROI+1,xROI:xROI+widthROI+1].sum()/area))     
    print('NORM',stack_im[0]/(stack_im[0][yROI:yROI+heightROI+1,xROI:xROI+widthROI+1].sum()/area))
    
    return(np.asarray(stack_im_ar),np.asarray(stack_ob_ar),stack_im[0][yROI:yROI+heightROI+1,xROI:xROI+widthROI+1].sum()/area)

def oscillation(stack_im,stack_ob,xROI=xROI,yROI=yROI,widthROI=widthROI,heightROI=heightROI,repeatedPeriod=False,folder=False,show=True,nnn='name'):
    """
...
#    PLOT oscillation
    ...
    if (0<=xROI<=im.shape[1] and 0<=xROI+widthROI<=im.shape[1] and 0<=yROI<=im.shape[0] and 0<=yROI+heightROI<=im.shape[0]):
        ...
        if folder:
            ...
            fig.savefig(str(folder)+'/oscillationPlot_X'+nnn+str(xROI)+'Y'+str(yROI)+'.png', bbox_inches='tight')
        plt.close('all')
    else:
        print('!!!WARNING!!! \nROI out of range')
...
def saveIm(ti,dpci,dfi,vis_map,name='name',folder='folder',overWrite=False):
    ...
    if not os.path.exists(folder+'ti\\'):
        makedirs(folder+'ti\\')
    fits.writeto(folder+'ti\\'+'/ti_'+str(name)+'.fits',ti,clobber=overWrite)
    if not os.path.exists(folder+'dpci\\'):
        makedirs(folder+'dpci\\')
    fits.writeto(folder+'dpci\\''/dpci_'+str(name)+'.fits',dpci,clobber=overWrite)
    if not os.path.exists(folder+'dfi\\'):
        makedirs(folder+'dfi\\')
    fits.writeto(folder+'dfi\\'+'/dfi_'+str(name)+'.fits',dfi,clobber=overWrite)
    if not os.path.exists(folder+'visi\\'):
        makedirs(folder+'visi\\')
    fits.writeto(folder+'visi\\'+'/visi_'+str(name)+'.fits',vis_map,clobber=overWrite)
